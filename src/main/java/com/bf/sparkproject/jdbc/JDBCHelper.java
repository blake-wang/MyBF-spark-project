...
public class JDBCHelper {
    ...
    //为什么要实现单例化呢？因为他的内部要封装一个简单的内部的数据库连接池
    //为了保证数据库连接池有且仅有一份，所以就通过单例的方式
    //保证JDBCHelp只有一个实例，实例中只有一份数据库连接池
     ...
     * 虽然使用PrepareStatement这种方式，可以只编译一次SQL，提高性能，但是，还是对于每次SQL都要向MYSQL发送一次网络请求
     * <p>
     * 可以通过批量执行SQL语句的功能优化这个性能
     * 一次性通过PrepareStatement发送多条SQL语句，比如100条，1000条，甚至上万条
     * 执行的时候，也仅仅编译一次就好
